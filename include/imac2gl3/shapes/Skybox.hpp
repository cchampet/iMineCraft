#ifndef _IMAC2GL3_Skybox_HPP_
#define _IMAC2GL3_Skybox_HPP_
...
class Skybox {
    ...
    void build(GLfloat height);
    
    // Pas de copie possible
    Skybox(const Skybox& c) {
    }
    
    Skybox& operator =(const Skybox& c) {
        return *this;
    }

    GLsizei insertPoint(ShapeVertex data, GLsizei index, float texCoordsX, float texCoordsY, float posX, float posY, float posZ, float normalX, float normalY, float normalZ);
public:
    // Constructeur: alloue le tableau de données et construit les attributs des vertex
    Skybox(GLfloat height):
        m_pDataPointer(0), m_nVertexCount(0) {
        build(height); // Construction (voir le .cpp)
    }
